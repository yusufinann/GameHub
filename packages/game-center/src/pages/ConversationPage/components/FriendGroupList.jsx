...
const FriendGroupList = ({
  ...
  // Avatar rengini grup adına göre belirleyen fonksiyon
  const getGroupColor = (name) => {
    ...
    // Grup adının ilk harfine göre renk seç
    const index = name.charCodeAt(0) % colorOptions.length;
    return colorOptions[index];
  };
  
  // Grup adının baş harflerini alan fonksiyon
  ...
  return (
    <Paper elevation={0} sx={{ bgcolor: 'background.paper', borderRadius: 2, overflow: 'hidden' }}>
      <List sx={{ p: 1 }}>
        ...
        {!friendGroupsLoading && friendGroups.map((friendGroup, index) => (
          <Fade in={true} timeout={300 + index * 100} key={friendGroup._id}>
            <ListItem
              button
              selected={selectedConversation?._id === friendGroup._id && selectedConversation?.type === 'friendGroup'}
            ...
            >
              <ListItemAvatar sx={{ minWidth: 'auto', mr: 2 }}>
                <Badge
                  ...
                  badgeContent={
                    isHost(friendGroup) ? (
                      <Tooltip title="Grup Yöneticisi" placement="top">
                        <StarIcon sx={{ 
              ...
              <ListItemText
                ...
                secondary={
                  <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
                    <GroupsIcon sx={{ fontSize: '0.85rem', mr: 0.5, color: theme.palette.text.secondary }} />
                    <Typography 
                      variant="body2" 
                      ...
                      sx={{ fontSize: '0.8rem' }}
                    >
                      {friendGroup.description || `${friendGroup.members?.length || 1} üye`}
                    </Typography>
                  </Box>
                }
              />
        ...
        {!friendGroupsLoading && friendGroups.length === 0 && (
          ...
          >
            ...
            <Typography variant="body1" fontWeight={500}>
              Henüz arkadaş grubunuz yok
            </Typography>
            <Typography variant="body2" sx={{ mt: 1, maxWidth: '80%', color: theme.palette.text.secondary }}>
              Yeni bir grup oluşturabilir veya bir gruba katılabilirsiniz
            </Typography>
          </Box>
        )}
      </List>
      ...
      >
        {isHost(selectedGroupForMenu) && (
          <MenuItem onClick={handleEditClick} sx={{ py: 1.2 }}>
            <ListItemIcon sx={{ minWidth: '34px', color: theme.palette.primary.main }}>
              <EditIcon fontSize="small" />
            </ListItemIcon>
            <Typography variant="body2">Grubu Düzenle</Typography>
          </MenuItem>
        )}
        
        {isHost(selectedGroupForMenu) && !isHost(selectedGroupForMenu) && <Divider sx={{ my: 0.5 }} />}
        
        {!isHost(selectedGroupForMenu) && selectedGroupForMenu && (
          <MenuItem onClick={handleLeaveGroupClick} sx={{ py: 1.2 }}>
            <ListItemIcon sx={{ minWidth: '34px', color: theme.palette.secondary.main }}>
              <ExitToAppIcon fontSize="small" />
            </ListItemIcon>
            <Typography variant="body2">Gruptan Ayrıl</Typography>
          </MenuItem>
        )}

        {isHost(selectedGroupForMenu) && (
          <MenuItem onClick={handleDeleteClick} sx={{ py: 1.2, color: theme.palette.error.main }}>
            <ListItemIcon sx={{ minWidth: '34px', color: 'inherit' }}>
              <DeleteIcon fontSize="small" />
            </ListItemIcon>
            <Typography variant="body2" color="inherit">Grubu Sil</Typography>
          </MenuItem>
        )}
      </Menu>
    </Paper>
