...
const VolumeCard = ({ 
  ...
  const cardBgGlow = theme.palette.mode === 'dark'
    ? `0 0 25px ${alpha(primary.dark, 0.5)}, 0 0 15px ${alpha(theme.palette.secondary.main, 0.3)}` 
    : `0 0 25px ${alpha(primary.main, 0.15)}, 0 0 15px ${alpha(theme.palette.secondary.main, 0.1)}`;
      ...
      <Paper 
        ...
        sx={{ 
          ...
          '&:hover': {
            ...
            boxShadow: `0 10px 30px ${alpha(theme.palette.warning.main, 0.2)}`
          }
        }}
      >
        <Box sx={{ 
          background: `linear-gradient(135deg, ${theme.palette.warning.main} 0%, ${primary.medium} 100%)`,
          p: 3,
          display: 'flex',
          alignItems: 'center',
          gap: 2
          ...
          <Box sx={{ 
            ...
            {soundEnabled ? <VolumeUpIcon fontSize="large" /> : <VolumeOffIcon fontSize="large" />}
          </Box>
            ...
            <Box sx={{ width: { xs: '100%', md: '33.33%' }, p: 1 }}>
              <Paper sx={{ 
                ...
                <Box sx={{ 
                  ...
                  <Switch size="small" defaultChecked={true} disabled={!soundEnabled} />
                </Box>
                <Slider 
                  size="small" 
                  defaultValue={80} 
            ...
            <Box sx={{ width: { xs: '100%', md: '33.33%' }, p: 1 }}>
              <Paper sx={{ 
                ...
                <Box sx={{ 
                  ...
                  <Typography variant="body2" fontWeight="bold">
                    {language === 'tr' ? 'Efekt Sesleri' : 'Sound Effects'}
                  </Typography>
                  <Switch size="small" defaultChecked={true} disabled={!soundEnabled} />
                </Box>
                <Slider 
            ...
            <Box sx={{ width: { xs: '100%', md: '33.33%' }, p: 1 }}>
              <Paper sx={{ 
                ...
                <Box sx={{ 
                  ...
                  <Typography variant="body2" fontWeight="bold">
                    {language === 'tr' ? 'Arayüz Sesleri' : 'UI Sounds'}
                  </Typography>
                  <Switch size="small" defaultChecked={true} disabled={!soundEnabled} />
                </Box>
                <Slider 
